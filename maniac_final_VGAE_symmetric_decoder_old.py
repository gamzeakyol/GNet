...
SUMMARY_WRITER = True
writer = SummaryWriter("./runs/maniac_graphconv_3layer_kernel_64_node_edge_alpha_0.3")
...
class Encoder(torch.nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv1 = pyg_nn.GraphConv(len(cfg.objects), 8 * out_channels, aggr="mean")
        self.conv2 = pyg_nn.GraphConv(8 * out_channels, 4 * out_channels, aggr="mean")
        ...
        self.conv_mu = pyg_nn.GraphConv(4 * out_channels, out_channels, aggr="mean")
        self.conv_logstd = pyg_nn.GraphConv(4 * out_channels, out_channels, aggr="mean")
...
class graphNet(torch.nn.Module):
    ...
    def forward(self, x, train_flag):
        ...
        self.actionpredictor = ActionPredictor(channels, node_len).to(dev)
...
for epoch in range(0, 200):
    ...
    for batch_idx, (graph) in enumerate(train_loader):
            ...
            net_loss = linkloss + 0.3 * nodeloss #+ kl_loss #+ actionloss
    ...
    print("Avg action score", avg_action_train, epoch, len(action_score_trains))
    ...
    for batch_idx, (graph) in enumerate(valid_loader):
        ...
        with torch.no_grad():
            ...
            val_link_ap = total_model.test(q_z, test_pos_edge_index, test_neg_edge_index)
            ...
            val_netloss = val_linkloss + 0.3 * val_nodeloss #+ kl_loss #+ action_val_loss #+ val_nodeloss 
            ...
            print('Epoch: {:3d}, Val NET loss: {:.4f}, Val node loss: , Val link loss: {:.4f}, AP: {:.4f}'.format(epoch, val_netloss, val_linkloss, val_link_ap))
    ...
    for batch_idx, (graph) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            #action_test_loss = nllloss(a_z[0].unsqueeze(0), gt_val_action.unsqueeze(0))

            kl_loss = total_model.model.kl_loss()

            net_losstest = linklosstest + 0.3 * nodelosstest #+ kl_loss #+ action_test_loss
